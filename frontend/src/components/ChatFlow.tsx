...
export function ChatFlow({ onSearch, disabled }: ChatFlowProps) {
  ...
  useEffect(() => {
    if (flowState === 'CLARIFYING' && questions.length > 0 && revealedIdx < questions.length - 1) {
      const timer = setTimeout(() => setRevealedIdx((prev) => prev + 1), 300)
  ...
  async function handleEnrichSearch(e: React.FormEvent) {
    ...
    try {
      ...
      triggerFinalSearch(data.enriched_query || query)
    } catch {
